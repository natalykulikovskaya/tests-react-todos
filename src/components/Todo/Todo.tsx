...
export const TodoItem = ({ todo }: Props) => {
    ...
    return (
        <li key={_id} className={className.todo} data-test-id="todo-li">
            <input type="checkbox" className={className.check} checked={status} onChange={() => changeCheckState({ _id, name, status })} data-testid="todo-check-input"/>
            <span className={cn(className.name, status && className.nameChecked)} data-testid="todo-name">{name}</span>
            <button type="button" className={className.delete} onClick={() => deleteTodo({ _id, name, status })} data-testid="todo-delete">x</button>
        </li>
    )
}